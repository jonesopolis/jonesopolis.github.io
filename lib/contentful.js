// Contentful Delivery API - server-side calls
...
export async function getPosts() {
  try {
    ...
    if (data.items?.length > 0) {
      // Filter to only include posts that have published=true or don't have the field set to false
      const resolved = resolveLinks(data.items, data.includes);
      return resolved.filter((post) => post.published !== false);
    }
...
export async function getPostBySlug(slug) {
  try {
    ...
    if (data.items?.length > 0) {
      const resolved = resolveLinks(data.items, data.includes);
      const post = resolved[0];
      // Only return if not explicitly unpublished
